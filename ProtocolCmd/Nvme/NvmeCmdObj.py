...
class NvmeCmdSet():
    def get_cns_desc(self, cns):
        if cns == NVME_ID_CNS_NS:
            return " -- Namespace"
        elif cns == NVME_ID_CNS_CTRL:
            return " -- Ctrller"
        return ""
    
    def nvme_cmd_identify(self, nsid, cns):
        cmd = NvmeCmdObj()
        cmd.desc = "Admin: Identify" +self.get_cns_desc(cns)
    ...
    def get_log_page_desc(self, logId):
        if logId == 2:
            return " -- SMART"
        return ""

    def nvme_cmd_get_log_page(self, nsid, logId, dwLen):
        if dwLen == 0:
            print("dwLen should not be 0", hex(dwLen))
        
        cmd = NvmeCmdObj()
        cmd.desc = "Admin: GetLogPage" + self.get_log_page_desc(logId)
    ...
    def nvme_cmd_lba_read(self, nsid, slba, secCnt):
        cmd = NvmeCmdObj()
        cmd.desc = "IO: Read"
    ...
    def nvme_cmd_lba_write(self, nsid, slba, secCnt):
        cmd = NvmeCmdObj()
        cmd.desc = "IO: Write"
    ...
    ##-------- ZNS --------
    def nvme_cmd_id_cns_zns(self, nsid):
        cmd = NvmeCmdObj()
    ...
    def nvme_cmd_set_zone(self, nsid, slba, zoneSetAction, selectionAll):
        cmd = NvmeCmdObj()
    ...
    def nvme_cmd_zone_append(self, nsid, slba, secCnt):
        cmd = NvmeCmdObj()
    ...
    def nvme_cmd_report_zone(self, nsid, slba, dataLen, zra, zrasf, isPartial):
        cmd = NvmeCmdObj()
    ...
    def get_cmd_colls(self):
        ...
        cmds.append(self.nvme_cmd_identify(1, NVME_ID_CNS_NS))
        cmds.append(self.nvme_cmd_identify(1, NVME_ID_CNS_CTRL))

        cmds.append(self.nvme_cmd_get_log_page(0, NVME_LOG_SMART, 1024))

        # io cmd
        cmds.append(self.nvme_cmd_lba_read(0, 0, 1))
        cmds.append(self.nvme_cmd_lba_write(0, 0, 1))

        #zns
        cmds.append(self.nvme_cmd_id_cns_zns(0))
        cmds.append(self.nvme_cmd_set_zone(0, 0, 0, 0))
        cmds.append(self.nvme_cmd_zone_append(0, 0, 1))
        cmds.append(self.nvme_cmd_report_zone(0, 0, 0x3F, 0, 0, True)) 
